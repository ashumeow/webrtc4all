...
HRESULT CMFSource::CopyVideoBuffer(UINT32 nWidth, UINT32 nHeight, const void* pBufferPtr, UINT32 nBufferSize)
{
	if(!pBufferPtr)
	{
		printf("Invalid buffer pointer"); // FIXME: TSK_DEBUG_ERROR()
		return E_POINTER;
	}

	if(!nWidth || !nHeight || !nBufferSize)
	{
		printf("Invalid parameter"); // FIXME: TSK_DEBUG_ERROR()
		return E_INVALIDARG;
	}
	if(m_pStream)
	{
	...
	{
		printf("No stream associated to this source"); // FIXME: TSK_DEBUG_ERROR()
	...
	m_nBufferSize(0)
{
	...
	if(majorType == MFMediaType_Video)
	{
		...
		{
			hr = E_NOTIMPL;
			printf("Video subType not supported"); // FIXME: TSK_DEBUG_ERROR()
			goto done;
		}
	...
	{
		hr = E_NOTIMPL;
		printf("Only video media type is supported"); // FIXME: TSK_DEBUG_ERROR()
		goto done;
	}
...
HRESULT CMFStreamSource::CopyVideoBuffer(UINT32 nWidth, UINT32 nHeight, const void* pBufferPtr, UINT32 nBufferSize)
{
	...
	{
		printf("Calling CopyVideoBuffer on no-video stream"); // FIXME: TSK_DEBUG_ERROR()
		return E_ILLEGAL_METHOD_CALL;
	}
	if(nWidth != m_structVideoParams.nWidth || nHeight != m_structVideoParams.nHeigh || nBufferSize != m_nBufferSize)
	{
		printf("Invalid argument %u#%u or %u#%u or %u#%u", nWidth, m_structVideoParams.nWidth, nHeight, m_structVideoParams.nHeigh, nBufferSize, m_nBufferSize); // FIXME: TSK_DEBUG_ERROR()
		return E_BOUNDS;
	}
